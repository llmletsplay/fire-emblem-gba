...
from src.utils.misc import parse_max_loops_fn
from src.utils.socket_utils import send_command
from src.services.websocket_service import broadcast_message, run_server_forever as start_websocket_service
from src.services.benchmark import load
from src.services.interactive import interactive_console
from src.core.llmdriver import run_auto_loop, MODEL
...
# Display which AI enhancements are enabled
log.info("=" * 60)
log.info("Fire Emblem AI - Enhancement Status")
log.info("-" * 60)
log.info(f"🧠 Persistent Learning: {'✅ ON' if config.PERSISTENT_LEARNING else '❌ OFF'}")
log.info(f"👁️ Anti-Hallucination: {'✅ ON' if config.ANTI_HALLUCINATION else '❌ OFF'}")
log.info(f"🎮 Smart Menu Escape: {'✅ ON' if config.SMART_MENU_ESCAPE else '❌ OFF'}")
log.info(f"🎯 Movement Coaching: {'✅ ON' if config.MOVEMENT_COACHING else '❌ OFF'}")
log.info(f"📋 Strategic Planning: {'✅ ON' if config.STRATEGIC_PLANNING else '❌ OFF'}")
log.info("=" * 60)
...
def start_mgba_with_scripting(rom_path=None, port=config.PORT):
    ...
    if not rom_path:
        log.error("ROM_FILE is not configured.")
        log.info(f"Set ROM_FILE in .env to FE7.gba or FE8.gba and place it in '{config.ROMS_FOLDER}'.")
        sys.exit(1)
    if not os.path.exists(rom_path):
        log.error(f"ROM file not found: {rom_path}")
        log.info(f"Please place your ROM file in the '{config.ROMS_FOLDER}' folder")
        sys.exit(1)
