...
--------------------------------------------------------------------------
-- Button Index (mGBA):
--   0:A  1:B  2:SELECT  3:START  4:RIGHT  5:LEFT  6:UP  7:DOWN  8:RT  9:LT
local KEY_INDEX = { A=0, B=1, SELECT=2, START=3, RIGHT=4, LEFT=5, UP=6, DOWN=7, RT=8, LT=9 }
local KEY_ALIAS = {
   -- D-pad
   U="UP", u="UP",  D="DOWN", d="DOWN",  
   LEFT="LEFT", left="LEFT", L="LEFT", l="LEFT",
   RIGHT="RIGHT", right="RIGHT", R="RIGHT", r="RIGHT",
   -- Shoulder/Triggers
   LT="LT", lt="LT", L_Shoulder="LT",
   RT="RT", rt="RT", R_Shoulder="RT",
   -- Menu
   S="START", s="SELECT",
}
local KEY_MASK = {}
...
--------------------------------------------------------------------------
--  UTILITY: CHECK GAME STATE (Fire Emblem GBA) --------------------------
--  Reads the PlaySt phase byte to detect whose turn it is.
--  Tries both FE7 (0x0202BC07, allegiance encoding) and
--  FE8 (0x0202BCF9, sequential encoding) addresses.
--  Returns: "player_phase", "enemy_phase", "npc_phase", or "unknown"
--------------------------------------------------------------------------

-- FE7 PlaySt phase (allegiance encoding: 0x00=player, 0x80=enemy, 0x40=NPC)
local FE7_PHASE_ADDR  = 0x0202BC07
-- FE8 PlaySt phase (sequential encoding: 0=player, 1=enemy, 2=NPC)
local FE8_PHASE_ADDR  = 0x0202BCF9

-- FE7 PlaySt turn counter (to distinguish "in-game" from "title screen")
local FE7_TURN_ADDR   = 0x0202BC08
-- FE8 PlaySt turn counter
local FE8_TURN_ADDR   = 0x0202BD00

-- FE7 player unit array base (non-zero char pointer = units loaded)
local FE7_PLAYER_BASE = 0x0202BD08
-- FE8 player unit array base
local FE8_PLAYER_BASE = 0x0202BE4C

local detected_game = nil  -- "fe7", "fe8", or nil (not yet detected)

local function detectGame()
   -- Read ROM header game code at 0x080000AC (4 bytes)
   local code = emu:readRange(0x080000AC, 4)
   if not code or #code < 4 then return nil end
   local code_str = code  -- mGBA returns string from readRange
   if code_str == "AE7E" then
      console:log("[INFO ] detectGame: FE7 (Blazing Blade) detected")
      return "fe7"
   elseif code_str == "BE8E" then
      console:log("[INFO ] detectGame: FE8 (Sacred Stones) detected")
      return "fe8"
   else
      console:log("[WARN ] detectGame: Unknown ROM code '" .. tostring(code_str) .. "'")
      return nil
   end
end

local function getPhaseAddr()
   if not detected_game then
      detected_game = detectGame()
   end
   if detected_game == "fe7" then
       return FE7_PHASE_ADDR, FE7_TURN_ADDR, FE7_PLAYER_BASE, "fe7"
    elseif detected_game == "fe8" then
       return FE8_PHASE_ADDR, FE8_TURN_ADDR, FE8_PLAYER_BASE, "fe8"
    end
...
-- menu: 0xCC51–0xCC52 (non-zero when menu open)
-- battle: 0xD057 (non-zero when in combat)
-- dialogue: 0xCC50, 0xCC54
--------------------------------------------------------------------------
...
local function isInDialogue()
   ...
   return (ptr_val == 0x5F and flg_val ~= 0x30)
end

local function getMenuSelection()
   local sel = emu:readRange(0xCC55, 1)
   if not sel then return -1 end
   return string.byte(sel, 1)
end

local function getMenuType()
   local menu_type = emu:readRange(0xCC53, 1)
   if not menu_type then return "unknown" end
   local mt = string.byte(menu_type, 1)
   -- Menu types based on FE7 disassembly
   if mt == 0x01 then return "unit" end       -- Unit menu (attack, item, etc.)
   if mt == 0x02 then return "item" end        -- Item menu
   if mt == 0x03 then return "trade" end       -- Trade menu
   if mt == 0x04 then return "support" end     -- Support conversation
   if mt == 0x05 then return "battle" end     -- Battle forecast
   if mt == 0x06 then return "repair" end      -- Weapon repair
   if mt == 0x07 then return "supply" end      -- Supply convoy
   if mt == 0x08 then return "arena" end       -- Arena
   if mt == 0x09 then return "vendor" end      -- Vendor/shop
   if mt == 0x0A then return "secret" end     -- Secret shop
   if mt == 0x0B then return "gameover" end    -- Game over
   if mt == 0x00 then return "none" end
   return string.format("0x%02X", mt)
end
...
local function getState()
   ...
   if isInMenu() then
      ...
      console:log("[DEBUG] getState: In menu type=" .. menu_type .. " selection=" .. menu_sel)
      return "menu:" .. menu_type .. ":" .. menu_sel
   end
   if isInDialogue() then
      console:log("[DEBUG] getState: In dialogue")
      return "dialogue"
   end

   -- Otherwise, return phase
   local phase_addr, turn_addr, player_base, game = getPhaseAddr()
   if not phase_addr then
      console:log("[WARN ] getState: Game not detected, returning unknown")
      return "unknown"
   end

   -- Read phase byte
   local phase_raw = emu:readRange(phase_addr, 1)
   if not phase_raw then
      return "unknown"
   end
   local phase_val = string.byte(phase_raw, 1)

   -- Read turn to check if we're actually in-game
   local turn_raw = emu:readRange(turn_addr, 1)
   local turn_val = turn_raw and string.byte(turn_raw, 1) or 0

   -- Check if player units are loaded (first unit slot has a non-zero char pointer)
   local unit_ptr = emu:readRange(player_base, 4)
   ...
   if not has_units and turn_val == 0 then
      console:log("[DEBUG] getState: No units loaded, turn=0 → not_in_game")
      return "not_in_game"
   end

   local state
   if game == "fe7" then
      -- FE7 allegiance encoding
      if phase_val == 0x00 then state = "player_phase"
      elseif phase_val == 0x80 then state = "enemy_phase"
      elseif phase_val == 0x40 then state = "npc_phase"
      else state = "other:" .. string.format("%02X", phase_val)
      end
   else
      -- FE8 sequential encoding
      if phase_val == 0 then state = "player_phase"
      elseif phase_val == 1 then state = "enemy_phase"
      elseif phase_val == 2 then state = "npc_phase"
      else state = "other:" .. string.format("%02X", phase_val)
      end
   end

   console:log("[DEBUG] getState: game=" .. game .. " phase=0x" .. string.format("%02X", phase_val) .. " turn=" .. turn_val .. " → " .. state)
   return state
end
...
local function parse(line, sock, sockId)
   ...
   if line:find(";") then
      ...
      if #toks > MAX_QUEUE_SIZE then
         console:log("[WARN ] parse: Queue truncated from " .. #toks .. " to " .. MAX_QUEUE_SIZE)
         local truncated = {}
         for i = 1, MAX_QUEUE_SIZE do truncated[i] = toks[i] end
         toks = truncated
      end
      if #toks > 0 then
    ...
    -- Game Memory Commands (works for FE7 or FE8 if loaded)
    if line_upper == "GAMESTATE" and sendGameState then
       console:log("[DEBUG] parse: GAMESTATE command received.")
       sendGameState(sock, sockId)
       return
    end
 
    if line_upper == "UNITS" and sendUnits then
       console:log("[DEBUG] parse: UNITS command received.")
       sendUnits(sock, sockId)
       return
    end

    if line_upper == "DETAIL" and sendDetailedState then
       console:log("[DEBUG] parse: DETAIL command received.")
   ...
   local save_slot_str, save_flags_str = line:match("^[Ss][Aa][Vv][Ee][Ss][Tt][Aa][Tt][Ee]%s+(%S+)%s*(%S*)$")
   if save_slot_str then
      console:log("[DEBUG] parse: SAVESTATE command received with slot: '" .. save_slot_str .. "'" .. ((save_flags_str and save_flags_str ~= "") and (" flags: '" .. save_flags_str .. "'") or " (default flags)"))
      ...
      if save_flags_str and save_flags_str ~= "" then
         local parsed_flags = tonumber(save_flags_str)
         if not parsed_flags then
            local err_msg = "Invalid flags value for SAVESTATE: '" .. save_flags_str .. "'"
            err(sockId, err_msg); sock:send("ERR " .. err_msg .. "\n"); return
         end
         if math.floor(parsed_flags) ~= parsed_flags then
            local err_msg = "Flags for SAVESTATE must be an integer: '" .. save_flags_str .. "'"
            err(sockId, err_msg); sock:send("ERR " .. err_msg .. "\n"); return
         end
         flags = parsed_flags
      end
      console:log("[DEBUG] parse: Calling emu:saveStateSlot(" .. slot .. ", " .. flags .. ")")
      local success = emu:saveStateSlot(slot, flags)
      if success then
         console:log("[INFO ] parse: SAVESTATE successful for slot " .. slot .. " with flags " .. flags)
         sock:send("OK SAVESTATE slot " .. slot .. "\n")
      else
         local err_msg = "emu:saveStateSlot failed for slot " .. slot .. " (flags " .. flags .. ")"
         err(sockId, err_msg); sock:send("ERR " .. err_msg .. "\n")
      end
      return
   end
